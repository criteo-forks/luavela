...
do --- Cannot sink, since nested inner table is non-PHI.
  local a, b = {{1}}, {{1}}
  for _=1,10000 do -- Need to force GC exit sometimes
    a = {{a[1][1]+b[1][1]}}
  end
  assert(a[1][1] == 10001)
end

do --- Cannot sink, as TNEW is CALLS argument, may be sinkable in future.
  local s = 0
  local immutable = require("ujit").immutable
  for i = 1,100 do
    local t = immutable{i}
    s = s + t[1]
  end
