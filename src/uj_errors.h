...
ERRDEF(SBUFOV, "string buffer length overflow")
...
ERRDEF(COTICKS, "cannot resume a coroutine with timeout from Lua-level")
...
ERRDEF(BADFREG,
       "no previously registered function, corrupted module definition?")
#if LJ_HASJIT
...
/* Sealing errors. */
ERRDEF(SEAL_BADTYPE, "attempt to seal an object of unsupported type \"%s\"")
ERRDEF(SEAL_FNUPVAL, "attempt to seal a function with upvalues")

/* Immutability errors. */
ERRDEF(IMMUT_BADTYPE,
       "attempt to make immutable an object of unsupported type \"%s\"")
ERRDEF(IMMUT_MODIF, "attempt to modify an immutable object")

/* Dumper errors. */
ERRDEF(DUMP_BADFILE, "unable to open \"%s\" for writing")
ERRDEF(DUMPBC_BADFN, "ujit.dump.bc: Function or proto expected")
ERRDEF(DUMPBCINS_BADPC, "ujit.dump.bcins: Invalid instruction index")

/* Profiler errors.  */
ERRDEF(PROF_START_BADIVL,
       "wrong profiling interval: number between 1 and 2^32 - 1 expected")
ERRDEF(PROF_START_BADFILE, "unable to open \"%s\" for writing")
ERRDEF(PROF_START_BADMODE,
 ...
 * awk -F, '/^ERRDEF/ { gsub(/ERRDEF./, ""); printf "fgrep -RI --include=\\*.h --include=\\*.c --include=\\*.dasc -q UJ_ERR_%s . || echo %s\n", $1, $1}' lj_errmsg.h | sh
 */
