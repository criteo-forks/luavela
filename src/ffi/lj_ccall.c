...
{
  size_t ngpr = cc->ngpr, nfpr = cc->nfpr;
  uint32_t i;
  for (i = 0; i < 2; i++) {
...
CTypeID lj_ccall_ctid_vararg(CTState *cts, const TValue *o)
{
  if (tvisnum(o)) {
    return CTID_DOUBLE;
  } else if (tviscdata(o)) {
                          ...
                          CCallState *cc)
{
  ...
  CTypeID fid;
  CType *ctr;
  size_t maxgpr, ngpr = 0, nsp = 0, narg;
#if CCALL_NARG_FPR
  size_t nfpr = 0;
#endif
  ...
  for (o = L->base+1, narg = 1; o < top; o++, narg++) {
    ...
    if (fid) {  /* Get argument type from field. */
      ...
      if (!(ct->info & CTF_VARARG))
        uj_err_caller(L, UJ_ERR_FFI_NUMARG);  /* Too many arguments. */
    ...
    if (CCALL_ALIGN_STACKARG && !rp && (d->info & CTF_ALIGN) > CTALIGN_PTR) {
      size_t align = (1u << ctype_align(d->info-CTALIGN_PTR)) -1;
      nsp = (nsp + align) & ~align;  /* Align argument on stack. */
    }
    if (nsp + n > CCALL_MAXSTACK) {  /* Too many arguments. */
    err_nyi:
      uj_err_caller(L, UJ_ERR_FFI_NYICALL);
    }
    dp = &cc->stack[nsp];
    nsp += n;
    isva = 0;
  ...
  if (fid) uj_err_caller(L, UJ_ERR_FFI_NUMARG);  /* Too few arguments. */
