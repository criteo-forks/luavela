...
static GCcdata *argv2cdata(jit_State *J, TRef tr, const TValue *o)
{
  GCcdata *cd;
  TRef trtypeid;
  if (!tref_iscdata(tr))
...
static CTypeID argv2ctype(jit_State *J, TRef tr, const TValue *o)
{
  if (tref_isstr(tr)) {
    GCstr *s = strV(o);
    CPState cp;
...
static size_t crec_copy_struct(CRecMemList *ml, CTState *cts, CType *ct)
{
  CTypeID fid = ct->sib;
  size_t mlp = 0;
  while (fid) {
...
static size_t crec_copy_unroll(CRecMemList *ml, CTSize len, CTSize step,
                              IRType tp)
{
  CTSize ofs = 0;
  size_t mlp = 0;
  if (tp == IRT_CDATA) tp = IRT_U8 + 2*lj_bsr(step);
  do {
    while (ofs + step <= len) {
      if (mlp >= CREC_COPY_MAXUNROLL) return 0;
      ml[mlp].ofs = ofs;
...
** UJ_TARGET_UNALIGNED: may emit unaligned loads/stores (not marked as such).
*/
static void crec_copy_emit(jit_State *J, CRecMemList *ml, size_t mlp,
                      ...
                      CType *ct)
{
  if (tref_isk(trlen)) {  /* Length must be constant. */
    ...
    rawcopy:
      needxbar = 1;
      if (UJ_TARGET_UNALIGNED || step >= CTSIZE_PTR)
        step = CTSIZE_PTR;
    }
...
static size_t crec_fill_unroll(CRecMemList *ml, CTSize len, CTSize step)
{
  CTSize ofs = 0;
  size_t mlp = 0;
  IRType tp = IRT_U8 + 2*lj_bsr(step);
  do {
    while (ofs + step <= len) {
      if (mlp >= CREC_COPY_MAXUNROLL) return 0;
      ml[mlp].ofs = ofs;
...
** UJ_TARGET_UNALIGNED: may emit unaligned stores (not marked as such).
*/
static void crec_fill_emit(jit_State *J, CRecMemList *ml, size_t mlp,
...
static TRef crec_ct_tv(jit_State *J, CType *d, TRef dp, TRef sp, const TValue *sval)
{
  CTState *cts = ctype_ctsG(J2G(J));
...
*/
static TRef crec_reassoc_ofs(jit_State *J, TRef tr, ptrdiff_t *ofsp, size_t sz)
{
  ...
  if (LJ_LIKELY(J->flags & JIT_F_OPT_FOLD) && irref_isk(ir->op2) &&
    ...
    ptrdiff_t k;
    if (irk->o == IR_KINT64)
      k = (ptrdiff_t)rawV(ir_kint64(irk)) * sz;
    else
...
static void crec_index_meta(jit_State *J, CTState *cts, CType *ct,
                            RecordFFData *rd, int is_newindex)
{
  CTypeID id = ctype_typeid(cts, ct);
  const TValue *tv = lj_ctype_meta(cts, id, is_newindex ? MM_newindex : MM_index);
  if (!tv)
    lj_trace_err(J, LJ_TRERR_BADTYPE);
  if (tvisfunc(tv)) {
    J->base[-1] = lj_ir_kfunc(J, funcV(tv)) | TREF_FRAME;
    rd->nres = -1;  /* Pending tailcall. */
  } else if (is_newindex == 0 && tvistab(tv) && tref_isstr(J->base[1])) {
    /* Specialize to result of __index lookup. */
    const TValue *o = lj_tab_get(J->L, tabV(tv), &rd->argv[1]);
...
static void recff_helper_cdata_index(jit_State *J, RecordFFData *rd, int is_newindex)
{
...
again:
  ...
  if (!sid) {
    ...
    crec_index_meta(J, cts, ct, rd, is_newindex);
    return;
  }
  ...
  if (is_newindex == 0) {  /* __index metamethod. */
...
{
  UNUSED(rd);
  recff_helper_cdata_index(J, rd, 0);
}

void recff_ffi_meta___newindex(jit_State *J, RecordFFData *rd)
{
  UNUSED(rd);
  recff_helper_cdata_index(J, rd, 1);
}

/* Record setting a finalizer. */
static void crec_finalizer(jit_State *J, TRef trcd, const TValue *fin)
{
...
static TRef crec_arith_int64(jit_State *J, TRef *sp, CType **s, enum MMS mm)
{
...
static TRef crec_arith_ptr(jit_State *J, TRef *sp, CType **s, enum MMS mm)
{
  CTState *cts = ctype_ctsG(J2G(J));
  CType *ctp = s[0];
  ...
  if (ctype_isptr(ctp->info) || ctype_isrefarray(ctp->info)) {
    if ((mm == MM_sub || mm == MM_eq || mm == MM_lt || mm == MM_le) &&
      ...
      if (mm == MM_sub) {  /* Pointer difference. */
        ...
        tr = emitir(IRT(IR_BSAR, IRT_INTP), tr, lj_ir_kint(J, lj_bsr(sz)));
...
static TRef crec_arith_meta(jit_State *J, TRef *sp, CType **s, CTState *cts,
                            RecordFFData *rd, enum MMS metamethod)
{
  ...
  if (J->base[0]) {
    if (tviscdata(&rd->argv[0])) {
      ...
      tv = lj_ctype_meta(cts, id, metamethod);
    }
  ...
  } else if (metamethod == MM_eq) {  /* Fallback cdata pointer comparison. */
    if (sp[0] && sp[1] && ctype_isnum(s[0]->info) == ctype_isnum(s[1]->info)) {
      /* Assume true comparison. Fixup and emit pending guard later. */
...
static void recff_helper_cdata_arith(jit_State *J, RecordFFData *rd, enum MMS metamethod)
{
  CTState *cts = ctype_ctsG(J2G(J));
  TRef sp[2];
  CType *s[2];
  ...
  {
    TRef tr;
    if (!(tr = crec_arith_int64(J, sp, s, metamethod)) &&
        !(tr = crec_arith_ptr(J, sp, s, metamethod)) &&
        !(tr = crec_arith_meta(J, sp, s, cts, rd, metamethod)))
      return;
    J->base[0] = tr;
    /* Fixup cdata comparisons, too. Avoids some cdata escapes. */
    if (J->postproc == LJ_POST_FIXGUARD && frame_iscont(J->L->base-1) &&
        !irt_isguard(J->guardemit)) {
      BCIns *pc = frame_contpc(J->L->base-1) - 1;
...
static void recff_helper_clib_index(jit_State *J, RecordFFData *rd, int is_index)
{
  ...
  if (tref_isudata(J->base[0]) && tref_isstr(J->base[1]) &&
    ...
    const TValue *tv = lj_tab_getstr(cl->cache, name);
    rd->nres = is_index;
    if (id && tv && !tvisnil(tv)) {
...
{
  UNUSED(rd);
  recff_helper_clib_index(J, rd, 1);
}

void recff_ffi_clib___newindex(jit_State *J, RecordFFData *rd)
{
  UNUSED(rd);
  recff_helper_clib_index(J, rd, 0);
}
...
/* Wrap errno macro into a function */
...
void recff_ffi_errno(jit_State *J, RecordFFData *rd)
{
  ...
  if (J->base[0])
    lj_trace_err(J, LJ_TRERR_NYICALL);
  J->base[0] = lj_ir_call(J, IRCALL_lj_crecord_errno, NULL);
}
...
static void recff_helper_ffi_xof(jit_State *J, RecordFFData *rd, enum fast_func func)
{
  CTypeID id = argv2ctype(J, J->base[0], &rd->argv[0]);
  if (func == FF_ffi_sizeof) {
    CType *ct = lj_ctype_rawref(ctype_ctsG(J2G(J)), id);
    if (ctype_isvltype(ct->info))
      lj_trace_err(J, LJ_TRERR_BADTYPE);
  } else if (func == FF_ffi_offsetof) {  /* Specialize to the field name. */
...
{
  UNUSED(rd);
  recff_helper_ffi_xof(J, rd, FF_ffi_sizeof);
}

void recff_ffi_alignof(jit_State *J, RecordFFData *rd)
{
  UNUSED(rd);
  recff_helper_ffi_xof(J, rd, FF_ffi_alignof);
}

void recff_ffi_offsetof(jit_State *J, RecordFFData *rd)
{
  UNUSED(rd);
  recff_helper_ffi_xof(J, rd, FF_ffi_offsetof);
}
