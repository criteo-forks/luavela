...
#define CTSUPPLDEF(_) \
  /* Supported pragmas (including the token "pragma" itself: */ \
  _(pragma         )  \
  _(pack           )  \
  _(pop            )  \
  _(push           )  \
  /* Supported GCC-specific values for __attribute__((...)): */ \
  _(__aligned__    )  \
  _(__mode__       )  \
  _(__packed__     )  \
  _(__vector_size__)  \
  ...
  _(packed         )  \
  _(vector_size    )  \
  /* Supported MSVC-specific value for __declspec(...): */ \
  _(align          )  \
  /* Recognised flags for ffi.abi for this implementation: */ \
  _(64bit          )  \
  _(fpu            )  \
  _(hardfp         )  \
  _(le             )  \
  /* End of the list of supplementary C tokens. */

enum ct_suppl_id {
#define CTSUPPLID(tok) CTOK_SUPPL_##tok,
  CTSUPPLDEF(CTSUPPLID)
#undef CTSUPPLID
  CTOK_SUPPL__MAX
};
...
typedef LJ_ALIGN(8) struct CCallback {
  ...
  size_t sizeid;                        /* Size of callback type table. */
  size_t topid;                 /* Highest unused callback type table slot. */
  size_t slot;                  /* Current callback slot. */
} CCallback;
...
struct CTState {
  ...
  size_t sizetab;       /* Size of C type table. */
  ...
  uint32_t suppl_hash[CTOK_SUPPL__MAX];
};
...
#define ctype_ctsG(g)           ((g)->ctype_state)
...
CTypeID lj_ctype_getname(CTState *cts, CType **ctp, GCstr *name,
                         uint32_t tmask);
CType *lj_ctype_getfieldq(CTState *cts, CType *ct, GCstr *name,
