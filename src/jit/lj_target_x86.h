...
#ifndef _UJ_TARGET_X86_H
#define _UJ_TARGET_X86_H
...
enum {
  ...
  RID_BASE = RID_R10D,          /* Interpreter BASE. */
  RID_LPC = RID_EBX,            /* Interpreter PC. */
  RID_DISPATCH = RID_R14D,      /* Interpreter DISPATCH table. */

  /* Internal register. Not allocated. */
  RID_INTERNAL = RID_R11D,
...
#define RSET_GPR        (RSET_RANGE(RID_MIN_GPR, RID_MAX_GPR)-RID2RSET(RID_ESP)-RID2RSET(RID_INTERNAL))
...
#define RSET_SCRATCH ((\
  ...
  RSET_RANGE(RID_ESI, RID_R11D+1)|RSET_FPR\
  ) & RSET_ALL)
...
#undef rset_picktop
#define rset_picktop(rs)        (lj_bsr(lj_bswap(rs)) ^ 0x18)
...
UJ_PEDANTIC_OFF
/*
 * enum members must be initialized with integer constant expressions,
 * but uint32_t is used in XO_* macros
 */
...
typedef enum {
  ...
  CC_E,   /* 0x4, ZF=1 */
  CC_NE,  /* 0x5, ZF=0 */
  CC_BE,  /* 0x6, CF=1 or ZF=1 */
  CC_NBE, /* 0x7, CF=0 and ZF=0 */
  CC_S,   /* 0x8, SF=1 */
  CC_NS,  /* 0x9, SF=0 */
  CC_P,   /* 0xa, PF=1 */
  CC_NP,  /* 0xb, PF=0 */
  CC_L,   /* 0xc, SF!=OF */
  CC_NL,  /* 0xd, SF=OF */
  CC_LE,  /* 0xe, ZF=1 or SF!=OF */
  CC_NLE, /* 0xf, ZF=0 and SF=OF */
  CC_C   = CC_B,
  CC_NAE = CC_C,
  CC_NC  = CC_NB,
  CC_AE  = CC_NB,
