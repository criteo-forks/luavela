...
static void strscan_double(uint64_t x, FpConv *o, int32_t ex2, int32_t neg) {
  double n;
...
static StrScanFmt strscan_hex(const uint8_t *p, FpConv *o,
...
/* Parse octal number. */
static StrScanFmt strscan_oct(const uint8_t *p, FpConv *o,
...
static StrScanFmt strscan_dec(const uint8_t *p, FpConv *o,
            StrScanFmt fmt, uint32_t opt,
  ...
  }
  {
    ...
    if (idig > 310/2) {
      FpConv tmp;
      tmp.u = neg? LJ_MINFINITY : LJ_PINFINITY;
      o->d = tmp.d;
      return fmt;
    }
    else if (idig < -326/2) { o->d = neg ? -0.0 : 0.0; return fmt; }
...
StrScanFmt strscan_tonumber(const uint8_t *p, double *d, uint32_t opt) {
  ...
  if (LJ_UNLIKELY(!lj_char_isdigit(*p))) {
    ...
    if (LJ_UNLIKELY(*p >= 'A')) {  /* Parse "inf", "infinity" or "nan". */
      FpConv tmp;
      tmp.u = LJ_NAN;
      if (lj_char_casecmp(p[0],'i') && lj_char_casecmp(p[1],'n') && lj_char_casecmp(p[2],'f')) {
        tmp.u = neg? LJ_MINFINITY : LJ_PINFINITY;
        p += 3;
        if (lj_char_casecmp(p[0],'i') && lj_char_casecmp(p[1],'n') && lj_char_casecmp(p[2],'i') &&
            lj_char_casecmp(p[3],'t') && lj_char_casecmp(p[4],'y')) { p += 5; }
      } else if (lj_char_casecmp(p[0],'n') && lj_char_casecmp(p[1],'a') && lj_char_casecmp(p[2],'n')) {
        p += 3;
      }
      while (lj_char_isspace(*p)) { p++; }
  ...
  /* Parse regular number. */
  {
    ...
    if (*p) {
      ...
      if (lj_char_casecmp(*p, 'i')) {
        if (!(opt & STRSCAN_OPT_IMAG)) { return STRSCAN_ERROR; }
        p++; fmt = STRSCAN_IMAG;
      } else if (fmt == STRSCAN_INT) {
        if (lj_char_casecmp(*p, 'u')) { p++, fmt = STRSCAN_U32; }
        if (lj_char_casecmp(*p, 'l')) {
          p++;
          if (lj_char_casecmp(*p, 'l')) { p++, fmt += STRSCAN_I64 - STRSCAN_INT; }
          else if (!(opt & STRSCAN_OPT_C)) { return STRSCAN_ERROR; }
          else if (sizeof(long) == 8) { fmt += STRSCAN_I64 - STRSCAN_INT; }
        }
        if (lj_char_casecmp(*p, 'u') && (fmt == STRSCAN_INT || fmt == STRSCAN_I64)) {
